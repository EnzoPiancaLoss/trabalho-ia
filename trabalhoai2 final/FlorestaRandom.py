...
from sklearn.metrics import accuracy_score, confusion_matrix, roc_curve, auc, f1_score, mean_squared_error, r2_score
from sklearn.preprocessing import LabelEncoder, StandardScaler

now = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
OUTPUT_DIR = f"output_rf_{now}"
...
# Random Forest
model = RandomForestClassifier(random_state=42, n_estimators=100, max_depth=15, class_weight="balanced" )
...
# Curva de overfitting (validation curve) para Random Forest
...
plt.ylabel("Acurácia")
plt.title("Curva de Overfitting - Random Forest")
plt.legend()
plt.grid(True)
plt.tight_layout()
...
# SSE (Soma dos Quadrados dos Erros) - para classificação, pode ser calculado como erro quadrático entre rótulos reais e previstos
sse = np.sum((y_test - y_pred) ** 2)

# SST (Soma Total dos Quadrados) - variância total dos dados em relação à média dos rótulos reais
...
with open(os.path.join(OUTPUT_DIR, "hiperparametros.txt"), "w", encoding="utf-8") as f:
    ...
    f.write(f"SSE (Soma dos Quadrados dos Erros): {sse:.4f}\n")
    f.write(f"SST (Soma Total dos Quadrados): {sst:.4f}\n")
